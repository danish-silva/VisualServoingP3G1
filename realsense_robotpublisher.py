...
# ====== Config ======
HOST = "127.0.0.1"
PORT = 5566                 # controller connects here
CB_COLS, CB_ROWS = 6, 5     # inner corners
LAMBDA = 0.5
...
# ----- TCP server (listen, accept, send on accepted socket) -----
...
def send_payload(payload: dict):
    ...
    while True:
        try:
            ...
            print("[INFO] Waiting for a new controller connection ...")
            conn, addr = srv.accept()
            print(f"[INFO] Re-established with {addr}")
    ...
    while True:
        ...
        if found and corners is not None:
            ...
            if valid.sum() >= 3:
                ...
                info = f"FOUND | ||e||={payload['err_px']:.1f}px"
